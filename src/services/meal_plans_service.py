...
class MealPlansService:
    ...
    def add_entry(self, id_plan: str, datos_entrada: dict) -> dict | None:
        ...
        ingredientes_receta = self.recipes_service.get_recipe_ingredients(datos_entrada['id_receta'])
        ids_reemplazables = {r['id_ingrediente'] for r in ingredientes_receta if r['rol'] == 'reemplazable'}
        ...
        if not ids_reemplazables.issubset(seleccionados):
            ...
            raise ValueError(f"Faltan ingredientes_seleccionados para ingredientes reemplazables: {faltantes}")
    ...
    def get_meal_list(self, id_plan: str) -> list[dict]:
        ...
        entradas_ordenadas = sorted(entradas, key=lambda e: (e['fecha'], e['tipo_comida']))
        result = []
        for entrada in entradas_ordenadas:
            receta = self.recipes_service.get_recipe(entrada['id_receta'], populate=False)
            result.append({
                'id': entrada['id'],
                'fecha': entrada['fecha'],
                'tipo_comida': entrada['tipo_comida'],
                'receta': receta,
                'ingredientes_seleccionados': entrada.get('ingredientes_seleccionados', {}),
            })
        return result

    def get_grocery_list(self, id_plan: str) -> dict:
        entradas = self.repo.get_entries(id_plan)
        agg_requeridos: dict[str, dict] = defaultdict(lambda: {'cantidad': 0.0, 'ingrediente': None})
        agg_opcionales: dict[str, dict] = defaultdict(lambda: {'cantidad': 0.0, 'ingrediente': None})

        all_ids_ingrediente = set()
        entradas_con_ingredientes = []

        for entrada in entradas:
            ingredientes_receta = self.recipes_service.get_recipe_ingredients(entrada['id_receta'])
            seleccionados = entrada.get('ingredientes_seleccionados', {})
            entradas_con_ingredientes.append((entrada, ingredientes_receta, seleccionados))
        ...
        for entrada, ingredientes_receta, seleccionados in entradas_con_ingredientes:
            for ri in ingredientes_receta:
                rol = ri['rol']
                ...
                if rol == 'requerido':
                    ...
                    agg_requeridos[id_elegido]['ingrediente'] = ingredientes_map.get(id_elegido)
        ...
        def agg_a_lista(agg: dict) -> list[dict]:
            return [
                {'ingrediente': v['ingrediente'], 'cantidad': v['cantidad'], 'unidad': v['unidad']}
                for v in agg.values()
            ]
